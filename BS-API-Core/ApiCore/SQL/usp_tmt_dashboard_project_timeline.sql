...
(
    @in_dtStartDate        DATE,        --ต้องส่งค่ามาเสมอ
    @in_dtEndDate          DATE,        --ต้องส่งค่ามาเสมอ			
    @in_vchProjectHeaderID INT = NULL,
    @in_xmlUserID          XML = NULL   --ส่งค่ามาเป็น list user id
...
BEGIN
    ...
    BEGIN
        INSERT INTO @tGetDataXML (user_id)
        ...
        IF EXISTS (SELECT 1 FROM @tGetDataXML)
            SET @hasUserFilter = 1;
    END
    ...
    ;WITH tProject AS (
        ...
        WHERE task.start_date <= @in_dtEndDate
            ...
            AND (
                @hasUserFilter = 0
                OR taskMem.user_id IN (SELECT user_id FROM @tGetDataXML)
            )
    ...
    OPTION (RECOMPILE); -- บังคับให้ SQL Server สร้าง execution plan ใหม่ทุกครั้ง
END
GO
