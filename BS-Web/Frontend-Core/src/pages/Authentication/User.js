...
const UserPage = (props) => {
  ...
  const handleOpenDelete = async (row) => {
    ...
    if (result && String(result.message_code) === "0") {
      ...
      BSAlertSwal2.show("error", result?.message_text || getResource(resourceData, "SaveFailed") || "Save failed");
    }
  };
  ...
  const handleChange = (eOrName, value) => {
    ...
    if (name === "email_address") {
      ...
      setEmailError(val && !emailPattern.test(val) ? getResource(resourceData, "InvalidEmail") || "Invalid email address" : "");
    }
  };
  ...
  const handleSave = async () => {
    if (!form.user_id || !form.user_group_id || !form.first_name || !form.last_name || !form.locale_id || !form.is_active) {
      BSAlertSwal2.show("warning", getResource(resourceData, "FillRequiredFields") || "Please fill all required fields.");
      return;
    }
  ...
  return (
    <>
          ...
          bsComboBox={[
            {
              ...
              Display: "name",
              Value: "name",
              Default: getResource(resourceData, "SelectUserGroup") || "--- Select User Group ---",
              PreObj: "sec",
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
        <DialogTitle>{editMode ? getResource(resourceData, "EditUser") || "Edit User" : getResource(resourceData, "AddUser") || "Add User"}</DialogTitle>
        <DialogContent>
        ...
        <DialogActions sx={{ display: "flex", justifyContent: "space-between" }}>
          ...
          <Box>
            ...
            <Button onClick={handleSave} variant="contained" color="primary">
              {editMode ? getResource(resourceData, "SaveChanges") || "Save Changes" : getResource(resourceData, "Add") || "Add"}
            </Button>
          </Box>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={isPopupResetPasswordOpen} onClose={() => setIsPopupResetPasswordOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{getResource(resourceData, "ResetPassword") || "Reset Password"}</DialogTitle>
        <DialogContent>
          <Typography variant="body2" color="text.secondary">
            {getResource(resourceData, "ConfirmResetPassword") || "Confirm resetting the user's password."}
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setIsPopupResetPasswordOpen(false)} color="primary">
            {getResource(resourceData, "Cancel") || "Cancel"}
          </Button>
          <Button onClick={sendChangePassword} color="primary" variant="contained">
      ...
      {/* New Password Popup */}
      <Dialog open={openPwDialog} onClose={() => setOpenPwDialog(false)} PaperProps={{ sx: { borderRadius: 3, padding: 2, minWidth: 350 } }}>
        ...
        <DialogActions sx={{ justifyContent: "center", paddingBottom: 2 }}>
          <Button variant="contained" color="primary" onClick={() => navigator.clipboard.writeText(newPassword)}>
            {getResource(resourceData, "Copy") || "Copy"}
          </Button>
