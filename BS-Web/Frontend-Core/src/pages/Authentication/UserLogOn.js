...
const UserLogOnPage = (props) => {
  ...
  const handleViewLogonDetails = (row) => {
    ...
    if (!isFinite(lat) || !isFinite(lon)) {
      BSAlertSwal2.show(
        ...
        getResourceByGroupAndName(
          ...
          "no_gps_available",
          locale_id,
        )?.resource_value || "No GPS coordinates available for this user.",
      );
      return;
    }
    ...
    setMapPoint({
      ...
      lon,
      acc,
      last_alive_time: row.last_alive_time || null,
    });
    setMapOpen(true);
  };
          ...
          bsCustomActions={[
            {
              label:
                getResourceByGroupAndName(
                  ...
                  "ClearLoggedOnUsers",
                  locale_id,
                )?.resource_value || "Clear Logged On Users",
              icon: <PersonRemoveIcon />,
              onClick: handleClearLogons,
              variant: "contained",
              sx: {
                bgcolor: "#4a56c5",
                color: "#fff",
      ...
      >
        ...
        <DialogContent dividers>
          <Typography variant="body2" gutterBottom>
            {mapPoint
              ? `Latitude: ${mapPoint.lat.toFixed(6)}, Longitude: ${mapPoint.lon.toFixed(6)}`
              : ""}
          </Typography>
          <Typography variant="body2" gutterBottom>
            {mapPoint && mapPoint.acc !== null
              ? `Accuracy: ${mapPoint.acc}`
              : ""}{" "}
            {mapPoint && mapPoint.last_alive_time !== null
              ? `| Last Alive Time: ${mapPoint.last_alive_time}`
              : ""}
          </Typography>
            ...
            {mapPoint ? (
              <iframe
                ...
                src={`https://www.openstreetmap.org/export/embed.html?bbox=${
                  mapPoint.lon - 0.01
                },${mapPoint.lat - 0.01},${mapPoint.lon + 0.01},${mapPoint.lat + 0.01}&layer=mapnik&marker=${mapPoint.lat},${mapPoint.lon}`}
                style={{ border: 0 }}
              />
            ) : (
