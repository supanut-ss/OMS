...
const TaskDialog = ({ phases, projectHeader, open, onClose, lang }) => {
    ...
    const requiredFields = projectHeader.record_type === "PROJECT" ? [
        "task_status",
        "task_name",
        "priority",
        "issue_type",
    ...
    const getLang = async () => {
        setResourceData(await getResources(projectHeader.record_type === "PROJECT" ? "t_tmt_project_task" : "t_tmt_project_task_ma", lang));
        setResourceDataProject(await getResources(projectHeader.record_type === "PROJECT" ? "t_tmt_project_header" : "t_tmt_project_header_ma", lang));
    }
    useEffect(() => {
        getLang()
        // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    return (
        <Dialog open={open} onClose={() => onClose(false)} fullScreen>
            ...
            <DialogContent sx={{ mt: 1 }}>
                ...
                {projectHeader.record_type === "PROJECT" && <TaskProject formData={formData} errors={errors} updateField={updateField} getResource={getResource} resourceData={resourceData} resourceDataProject={resourceDataProject} projectHeader={projectHeader} renderInput={renderInput} />}
                {projectHeader.record_type === "MA" && <TaskMa formData={formData} errors={errors} updateField={updateField} getResource={getResource} resourceData={resourceData} resourceDataProject={resourceDataProject} projectHeader={projectHeader} renderInput={renderInput} />}
                {/* Tabs */}
                {formData.project_task_id && (
                    <Box>
                        <Tabs value={tap} onChange={(e, v) => setTap(v)} sx={{ mt: 3 }}>
                            ...
                            {projectHeader.record_type === "MA" && (<Tab label={getResource(resourceData, "upload_image")} />)}
                        </Tabs>
